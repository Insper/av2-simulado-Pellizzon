...
t_but but0 = {.width = BUT_SIZE, .height = BUT_SIZE,
  ...
  .x = BUT_SIZE/2, .y = ILI9488_LCD_HEIGHT - BUT_SIZE/2,
.status = 1, .name="ON/OFF"};

t_but but1 = {.width = BUT_SIZE, .height = BUT_SIZE,
  .colorOn = COLOR_VIOLET, .colorOff = COLOR_DARKVIOLET,
  .x = BUT_SIZE/2 + BUT_SIZE, .y = ILI9488_LCD_HEIGHT - BUT_SIZE/2,
.status = 1, .name="UP"};

t_but but2 = {.width = BUT_SIZE, .height = BUT_SIZE,
  .colorOn = COLOR_MAGENTA, .colorOff = COLOR_GRAY,
  .x = BUT_SIZE/2 + 2*BUT_SIZE, .y = ILI9488_LCD_HEIGHT - BUT_SIZE/2,
.status = 1, .name="DOWN"};

t_but but3 = {.width = BUT_SIZE, .height = BUT_SIZE,
  .colorOn = COLOR_GREEN, .colorOff = COLOR_DARKGREEN,
  .x = BUT_SIZE/2 + 3*BUT_SIZE, .y = ILI9488_LCD_HEIGHT - BUT_SIZE/2,
.status = 1, .name="FIR"};

t_but but4 = {.width = BUT_SIZE, .height = BUT_SIZE,
	.colorOn = COLOR_ORANGE, .colorOff = COLOR_CYAN,
	.x = BUT_SIZE/2 + 4*BUT_SIZE, .y = ILI9488_LCD_HEIGHT - BUT_SIZE/2,
.status = 1, .name="ScaleX"};
t_but but5 = {.width = BUT_SIZE, .height = BUT_SIZE,
	.colorOn = COLOR_BLUE, .colorOff = COLOR_CYAN,
	.x = BUT_SIZE/2 + 5*BUT_SIZE, .y = ILI9488_LCD_HEIGHT - BUT_SIZE/2,
.status = 0, .name="5hz"};
...
const float32_t alta_firCoeffs32[NUM_TAPS] = {
	0.07242382057239563,
	0.05968051027848589,
	-0.08952076541772881,
	-0.28969528228958263,
...
void draw_button(t_but but){
  ...
  ili9488_draw_string(but.x-but.width/2 + 5, but.y-10, but.name);
}
...
void task_lcd(void){
  ...
  t_but botoes[] = {but0, but1, but2, but3, but4, but5};
  ...
  while (true) {
    if (xQueueReceive( xQueueTouch, &(touch), ( TickType_t )  0 / portTICK_PERIOD_MS)) {
	  ...
	  if (botoes[4].status == 1){
		  scale_x = 10;
	  }
	  
	  if (botoes[4].status == 0){
		  scale_x = 6;
	  }
	  
	  if (botoes[5].status == 0){
		  filtro = 0;
	  }
	  
	  if (botoes[5].status == 1){
		  filtro = 1;
	  }
    ...
    if (xQueueReceive( xQueuePlot, &(plot), ( TickType_t )  100 / portTICK_PERIOD_MS)) {
      ...
      if(botoes[0].status){
        ...
        if(botoes[3].status){
          ...
          ili9488_draw_filled_circle(x, ILI9488_LCD_HEIGHT-BUT_SIZE - plot.filtrado / scale, 4 );
        }
        else{
			if(botoes[5].status){
				...
				ili9488_set_foreground_color(COLOR_CONVERT(COLOR_BLACK));
				ili9488_draw_filled_circle(x, ILI9488_LCD_HEIGHT-BUT_SIZE - plot.raw / scale, 2 );
			}
        }
      }
    }
...
void task_adc(void){
  ...
  float32_t alta_firStateF32[BLOCK_SIZE + NUM_TAPS - 1];
  float32_t alta_inputF32[BLOCK_SIZE + NUM_TAPS - 1];
  float32_t alta_outputF32[BLOCK_SIZE + NUM_TAPS - 1];
  arm_fir_instance_f32 alta;
  arm_fir_init_f32(&alta, NUM_TAPS, (float32_t *)&alta_firCoeffs32[0], &alta_firStateF32[0], BLOCK_SIZE);
  ...
  while(1){
    if (xQueueReceive( xQueueADC, &(adc), 100)) {
		if (filtro) {
			printf("sou filtro de 5 hz\n");
			if(i <= NUM_TAPS){
				alta_inputF32[i++] = (float) adc.value;
			} else {
				arm_fir_f32(&S, &alta_inputF32[0], &alta_outputF32[0], BLOCK_SIZE);
				plot.raw = (int) alta_inputF32[0];
				plot.filtrado = (int) alta_outputF32[0];
